...
public class TestDataController {
    ...
    public ResponseEntity<?> createQrCodeNpid() {
       try {
            PresentationUrlDTO presentationUrlDTO = testDataService.createTestNpid();
    ...
    public ResponseEntity<?> createQrCodeEucc() {

        try {
            PresentationUrlDTO presentationUrlDTO = testDataService.createTestEucc();
            return ResponseEntity.ok(presentationUrlDTO);
        } catch (Exception e) {
            log.error("Error creating QR code: {}", e.getMessage(), e);
            return ResponseEntity.status(500).body(e.getMessage());
